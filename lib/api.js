export const getCategory = async(slug) => {
  const { default: { attributes, html } } = await import(`../content/categories/${slug}.md`);
...
export const getProject = async(slug) => {
  const { default: { attributes, html } } = await import(`../content/projects/${slug}.md`);
  return { attributes, html };
};
...
export const getProjectsByTag = async tag => {
  ...
  for (const key of context.keys()) {
    ...
    if (attributes.tags?.includes(tag)) {
      projects.push({
        slug: project.replace(".md", ""),
        title: attributes.title,
        date: attributes.date,
